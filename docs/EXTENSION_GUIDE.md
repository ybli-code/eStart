...
### 4.1 Edge/Chrome 提示 "无法对图像解释" (Image could not be decoded)
如果您在安装 `.crx` 文件时遇到此错误，通常是因为浏览器无法正确解析 SVG 格式的图标。
**解决方案：**
1. **优先使用解压安装**：按照 2.2 节的方法，解压 `.zip` 后使用“加载解压缩的扩展”安装。
2. **替换 PNG 图标**：在项目根目录的 `public/` 文件夹中放入 `icon-16.png`, `icon-48.png`, `icon-128.png` 三个文件，然后重新运行 `npm run build:extension`。构建脚本会自动检测并优先使用 PNG 格式。

### 4.2 扩展程序无图标显示
如果安装后在扩展管理页面看不到图标，请检查：
- 确保 `public/` 目录下存在 `favicon.svg` 或上述 PNG 图标。
- 如果是离线安装，尝试刷新页面或重新加载扩展。
